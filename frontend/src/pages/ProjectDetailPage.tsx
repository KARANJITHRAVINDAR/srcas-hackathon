...
export default function ProjectDetailPage() {
    ...
    // Audit Report states
    const [latestAuditReport, setLatestAuditReport] = useState<any>(null);
    const [auditReportHistory, setAuditReportHistory] = useState<any[]>([]);
    ...
    const handleGenerateAuditReport = async () => {
        ...
        try {
            ...
            showAlert({ type: 'error', title: 'Generation Failed / Discrepancy Detected', message: err.response?.data?.error || err.response?.data?.message || 'Failed to generate audit report.' });
        } finally {
            ...
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-6">
                        ...
                        {/* Official Audit Report (Blockchain Merkle Verification) Widget */}
                        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                            <div className="flex justify-between items-center">
                                <h3 className="text-lg font-black text-slate-900 flex items-center gap-2">
                                    <FileText className="text-indigo-600" size={20} /> Official Audit Report
                                </h3>
                                <span className={`text-xs font-black px-2.5 py-1 rounded-full ${
                                    ...
                                    {project?.status === 'CLOSED' || project?.status === 'COMPLETED' ? 'AVAILABLE' : 'GATED AT CLOSURE'}
                                </span>
                            </div>

                            {project?.status !== 'CLOSED' && project?.status !== 'COMPLETED' ? (
                                <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-600 font-medium">
                                    🔒 <strong>Official Audit Report is gated.</strong> Once the project is fully <code>CLOSED</code> (beneficiary feedback met + closure video verified), the official cryptographically signed audit report can be generated.
                                </div>
                            ) : (
                                <div className="space-y-4">
                                    {latestAuditReport ? (
                                        <div className="p-4 bg-slate-900 text-white rounded-xl space-y-3 shadow-md">
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    <div className="text-xs font-bold text-slate-400">Audit Report — Version {latestAuditReport.reportVersion}</div>
                                                    <div className="text-xs text-slate-300 mt-0.5">Generated: {new Date(latestAuditReport.generatedAt).toLocaleString()}</div>
                                                </div>
                                                <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase flex items-center gap-1 ${
                                                    latestAuditReport.onchainVerificationStatus === 'ALL_VERIFIED'
                                                        ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'
                                                        : 'bg-rose-500/20 text-rose-400 border border-rose-500/30'
                                                }`}>
                                                    <ShieldCheck size={12} />
                                                    {latestAuditReport.onchainVerificationStatus === 'ALL_VERIFIED' ? '✓ Polygon Amoy Verified' : '⚠️ Discrepancy Found'}
                                                </span>
                                            </div>

                                            <div className="text-xs text-slate-400 font-mono bg-slate-950 p-2.5 rounded-lg border border-slate-800 break-all">
                                                <span className="text-slate-500 block text-[10px] uppercase font-sans font-bold">Signer Public Wallet:</span>
                                                {latestAuditReport.signerWalletAddress}
                                            </div>

                                            {latestAuditReport.onchainVerificationStatus === 'MISMATCH_FOUND' ? (
                                                <div className="p-3 bg-rose-950/80 border border-rose-800 text-rose-200 text-xs rounded-lg space-y-1">
                                                    <strong className="text-rose-400 flex items-center gap-1">
                                                        <AlertTriangle size={14} /> On-Chain Discrepancy Detected
                                                    </strong>
                                                    <p className="text-[11px] leading-relaxed">
                                                        Live on-chain verification detected a discrepancy between the database Merkle root and Polygon network state. Report download is blocked pending investigation.
                                                    </p>
                                                </div>
                                            ) : (
                                                <div className="flex gap-2 pt-1">
                                                    ...
                                                    <button
                                                        ...
                                                        className="bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 font-bold py-2.5 px-3 rounded-lg text-xs transition flex items-center justify-center gap-1"
                                                    >
                                    ...
                                    {isFunder && (
                                        <button
                                            ...
                                            disabled={isGeneratingAuditReport}
                                            className="w-full bg-[#10172A] hover:bg-slate-800 text-white py-3 rounded-xl font-extrabold text-xs flex items-center justify-center gap-2 shadow-md transition"
                                        >
                                            {isGeneratingAuditReport ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Award className="w-4 h-4 text-emerald-400" />}
                                            {latestAuditReport ? 'Re-Generate & Re-Verify Audit Report' : 'Generate Official Audit Report'}
                                        </button>
                                    )}
                                </div>
                            )}
