...
import { 
    ...
    CheckCheck, Clock, ArrowRight, Sparkles, Coins, Menu, X
} from 'lucide-react';
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] flex font-sans selection:bg-[#00A875]/20">
            ...
            {/* SIDEBAR (Desktop Fixed Rail / Mobile Slide-Out Drawer) */}
            <aside className={`w-64 bg-white border-r border-[#DDE3EA] flex flex-col fixed inset-y-0 left-0 z-50 shadow-[4px_0_24px_rgba(0,0,0,0.02)] transition-transform duration-300 ease-in-out ${
                isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
            }`}>
                {/* Brand & Mobile Close */}
                <div className="h-16 flex items-center justify-between px-6 border-b border-[#DDE3EA]">
                    <div className="flex items-center cursor-pointer" onClick={() => { navigate('/'); setIsMobileMenuOpen(false); }}>
                    ...
                    <button 
                        onClick={() => setIsMobileMenuOpen(false)}
                        className="p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg md:hidden transition"
            ...
            {/* MAIN CONTENT AREA */}
            <main className="flex-1 ml-0 md:ml-64 flex flex-col min-h-screen w-full min-w-0">
                
                {/* TOP BAR */}
                <header className="h-16 bg-white border-b border-[#DDE3EA] flex items-center justify-between px-4 sm:px-8 sticky top-0 z-30 shadow-[0_4px_24px_rgba(0,0,0,0.02)]">
                    <div className="flex items-center gap-3">
                        {/* Hamburger Button (Mobile Only) */}
                        <button 
                            onClick={() => setIsMobileMenuOpen(true)}
                            className="p-2 -ml-1 text-[#52627A] hover:text-[#10172A] rounded-xl hover:bg-slate-100 md:hidden transition flex items-center justify-center"
                    ...
                    <div className="flex items-center gap-3 sm:gap-6">
                        ...
                        <div className="relative" ref={dropdownRef}>
                            ...
                            {/* Dropdown Popover */}
                            {showDropdown && (
                                <div className="absolute right-[-40px] sm:right-0 mt-3 w-[calc(100vw-2rem)] sm:w-96 max-w-sm bg-white rounded-2xl shadow-2xl border border-[#DDE3EA] z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                        ...
                        {/* User Profile */}
                        <div className="flex items-center gap-2 sm:gap-3 border-l border-[#DDE3EA] pl-3 sm:pl-6">
                            <div className="w-8 h-8 rounded-full bg-[#10172A] flex items-center justify-center text-white shrink-0">
